...
import { normalizeAddExpenseArgs, formatCategoryList, createOpenAIClient, isWriteToolName } from "./utils";
import { handleWhatsAppVerification, handleWhatsAppMessage } from "./whatsapp-handler";
import {
  AI_MODEL,
  MAX_LLM_STEPS,
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      const { messages: rawMessages, analyticsMode } = (await req.json()) as ChatRequestBody;
      const history = (rawMessages ?? []) as OpenAI.Chat.Completions.ChatCompletionMessageParam[];
      ...
      const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = analyticsMode
      ...
      for (let step = 0; step < MAX_LLM_STEPS; step++) {
        ...
        for (const tc of msg.tool_calls) {
          ...
          if (isWriteToolName(tc.function.name)) {
            if (tc.function.name === TOOL_NAME.ADD_EXPENSE) {
              pendingWriteToolCalls.push({ toolName: tc.function.name, args: normalizeAddExpenseArgs(args) });
            } else {
              pendingWriteToolCalls.push({ toolName: tc.function.name, args: { id: Number(args.id) } });
            }
        ...
        if (pendingWriteToolCalls.length > 0) {
          return jsonResponse({ message: null, pendingToolCalls: pendingWriteToolCalls }, 200, cors);
        }

        messages.push(...toolResults);
      }

      return jsonResponse({ message: lastResponseText, pendingToolCalls: null }, 200, cors);
    } catch (err) {
      console.error(err);
